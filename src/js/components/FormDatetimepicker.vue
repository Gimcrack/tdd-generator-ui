...
<script>
    ...
    export default {
        ...
        watch : {
            value( val ) {
                this.selected = moment(val).format('YYYY-MM-DD HH:mm');
            }
        },
        ...
        data() {
            return {
                selected : this.value ? moment(this.value).format('YYYY-MM-DD HH:mm') : null,
                help_message : '',
                validation : {
                    settings : [],
                    rules : [],
        ...
        methods : {
            ...
            update(selected) {
                return Bus.$emit('UpdateFormControl', { key : this.name, value : selected });
            },
        },
